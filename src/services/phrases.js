export const phraseCategories = {
  navigation: {
    title: "Navigation & Directions",
    phrases: [
      {
        en: "Where is the Kaaba?",
        ar: "أين الكعبة؟",
        ur: "کعبہ کہاں ہے؟",
        hi: "काबा कहाँ है?",
        id: "Di mana Ka'bah?",
        ms: "Di manakah Kaabah?",
        tr: "Kabe nerede?",
        fa: "کعبه کجاست؟",
        fr: "Où est la Kaaba ?",
        de: "Wo ist die Kaaba?",
      },
      {
        en: "How do I get to Mina?",
        ar: "كيف أصل إلى منى؟",
        ur: "منیٰ تک کیسے پہنچوں؟",
        hi: "मिना तक कैसे पहुंचें?",
        id: "Bagaimana cara ke Mina?",
        ms: "Bagaimana cara ke Mina?",
        tr: "Mina'ya nasıl giderim?",
        fa: "چگونه به منا بروم؟",
        fr: "Comment aller à Mina ?",
        de: "Wie komme ich nach Mina?",
      },
      {
        en: "Which way to Arafat?",
        ar: "أي طريق إلى عرفات؟",
        ur: "عرفات کا راستہ کون سا ہے؟",
        hi: "अरफात का रास्ता कौन सा है?",
        id: "Jalan mana yang menuju Arafah?",
        ms: "Jalan mana yang menuju ke Arafah?",
        tr: "Arafat'a hangi yoldan gidilir?",
        fa: "کدام مسیر به عرفات می‌رود؟",
        fr: "Quel chemin pour Arafat ?",
        de: "Welcher Weg führt nach Arafat?",
      },
      {
        en: "Where is the Masjid al-Haram?",
        ar: "أين المسجد الحرام؟",
        ur: "مسجد الحرام کہاں ہے؟",
        hi: "मस्जिद अल-हरम कहाँ है?",
        id: "Di mana Masjidil Haram?",
        ms: "Di manakah Masjidil Haram?",
        tr: "Mescid-i Haram nerede?",
        fa: "مسجد الحرام کجاست؟",
        fr: "Où est la Mosquée sacrée ?",
        de: "Wo ist die Heilige Moschee?",
      },
      {
        en: "How do I get to Safa and Marwa?",
        ar: "كيف أصل إلى الصفا والمروة؟",
        ur: "صفا اور مروہ تک کیسے پہنچوں؟",
        hi: "सफा और मरवा तक कैसे पहुंचें?",
        id: "Bagaimana cara ke Safa dan Marwa?",
        ms: "Bagaimana cara ke Safa dan Marwah?",
        tr: "Safa ve Merve'ye nasıl giderim?",
        fa: "چگونه به صفا و مروه بروم؟",
        fr: "Comment aller à Safa et Marwa ?",
        de: "Wie komme ich zu Safa und Marwa?",
      },
      {
        en: "Where is the nearest bus stop?",
        ar: "أين أقرب محطة حافلات؟",
        ur: "قریب ترین بس اسٹاپ کہاں ہے؟",
        hi: "निकटतम बस स्टॉप कहाँ है?",
        id: "Di mana halte bus terdekat?",
        ms: "Di manakah perhentian bas yang terdekat?",
        tr: "En yakın otobüs durağı nerede?",
        fa: "نزدیک‌ترین ایستگاه اتوبوس کجاست؟",
        fr: "Où est l'arrêt de bus le plus proche ?",
        de: "Wo ist die nächste Bushaltestelle?",
      },
      {
        en: "Where is the nearest mosque?",
        ar: "أين أقرب مسجد؟",
        ur: "قریب ترین مسجد کہاں ہے؟",
        hi: "निकटतम मस्जिद कहाँ है?",
        id: "Di mana masjid terdekat?",
        ms: "Di manakah masjid yang terdekat?",
        tr: "En yakın cami nerede?",
        fa: "نزدیک‌ترین مسجد کجاست؟",
        fr: "Où est la mosquée la plus proche ?",
        de: "Wo ist die nächste Moschee?",
      },
      {
        en: "How do I get to the Grand Mosque?",
        ar: "كيف أصل إلى المسجد الحرام؟",
        ur: "مسجد الحرام تک کیسے پہنچوں؟",
        hi: "महान मस्जिद तक कैसे पहुंचें?",
        id: "Bagaimana cara ke Masjidil Haram?",
        ms: "Bagaimana cara ke Masjidil Haram?",
        tr: "Büyük Cami'ye nasıl giderim?",
        fa: "چگونه به مسجد الحرام بروم؟",
        fr: "Comment aller à la Grande Mosquée ?",
        de: "Wie komme ich zur Großen Moschee?",
      },
      {
        en: "Where is the nearest metro station?",
        ar: "أين أقرب محطة مترو؟",
        ur: "قریب ترین میٹرو اسٹیشن کہاں ہے؟",
        hi: "निकटतम मेट्रो स्टेशन कहाँ है?",
        id: "Di mana stasiun metro terdekat?",
        ms: "Di manakah stesen metro yang terdekat?",
        tr: "En yakın metro istasyonu nerede?",
        fa: "نزدیک‌ترین ایستگاه مترو کجاست؟",
        fr: "Où est la station de métro la plus proche ?",
        de: "Wo ist die nächste U-Bahn-Station?",
      },
    ],
  },
  rituals: {
    title: "Hajj & Umrah Rituals",
    phrases: [
      {
        en: "How do I perform Tawaf?",
        ar: "كيف أؤدي الطواف؟",
        ur: "طواف کیسے کریں؟",
        hi: "तवाफ कैसे करें?",
        id: "Bagaimana cara melakukan Tawaf?",
        ms: "Bagaimana cara melakukan Tawaf?",
        tr: "Tavaf nasıl yapılır?",
        fa: "چگونه طواف کنم؟",
        fr: "Comment effectuer le Tawaf ?",
        de: "Wie führe ich den Tawaf durch?",
      },
      {
        en: "Where is the nearest prayer area?",
        ar: "أين أقرب مكان للصلاة؟",
        ur: "قریب ترین نماز کی جگہ کہاں ہے؟",
        hi: "निकटतम प्रार्थना स्थल कहाँ है?",
        id: "Di mana tempat shalat terdekat?",
        ms: "Di manakah tempat solat yang terdekat?",
        tr: "En yakın namaz alanı nerede?",
        fa: "نزدیک‌ترین محل نماز کجاست؟",
        fr: "Où est la zone de prière la plus proche ?",
        de: "Wo ist der nächste Gebetsbereich?",
      },
      {
        en: "How do I perform Sa'i?",
        ar: "كيف أؤدي السعي؟",
        ur: "سعی کیسے کریں؟",
        hi: "सई कैसे करें?",
        id: "Bagaimana cara melakukan Sa'i?",
        ms: "Bagaimana cara melakukan Sa'ie?",
        tr: "Sa'y nasıl yapılır?",
        fa: "چگونه سعی کنم؟",
        fr: "Comment effectuer le Sa'i ?",
        de: "Wie führe ich den Sa'i durch?",
      },
      {
        en: "When is the time for Wuquf?",
        ar: "متى وقت الوقوف؟",
        ur: "وقوف کا وقت کب ہے؟",
        hi: "वुकूफ का समय कब है?",
        id: "Kapan waktu Wuquf?",
        ms: "Bilakah masa Wuquf?",
        tr: "Vakfe zamanı ne zaman?",
        fa: "زمان وقوف چه وقت است؟",
        fr: "Quand est le moment du Wuquf ?",
        de: "Wann ist die Zeit für Wuquf?",
      },
      {
        en: "How do I perform Rami?",
        ar: "كيف أؤدي رمي الجمرات؟",
        ur: "رمی جمرات کیسے کریں؟",
        hi: "रमी कैसे करें?",
        id: "Bagaimana cara melakukan Rami?",
        ms: "Bagaimana cara melakukan Rami?",
        tr: "Rami nasıl yapılır?",
        fa: "چگونه رمی کنم؟",
        fr: "Comment effectuer le Rami ?",
        de: "Wie führe ich den Rami durch?",
      },
      {
        en: "Where can I perform Tahallul?",
        ar: "أين يمكنني أداء التحلل؟",
        ur: "تحلل کہاں کریں؟",
        hi: "तहल्लुل कहाँ करें?",
        id: "Di mana saya bisa melakukan Tahallul?",
        ms: "Di mana saya boleh melakukan Tahallul?",
        tr: "Tahallul nerede yapılır?",
        fa: "کجا می‌توانم تحلل کنم؟",
        fr: "Où puis-je effectuer le Tahallul ?",
        de: "Wo kann ich Tahallul durchführen?",
      },
      {
        en: "How do I perform Umrah?",
        ar: "كيف أؤدي العمرة؟",
        ur: "عمرہ کیسے ادا کریں؟",
        hi: "उमरा कैसे करें?",
        id: "Bagaimana cara melakukan Umrah?",
        ms: "Bagaimana cara melakukan Umrah?",
        tr: "Umre nasıl yapılır?",
        fa: "چگونه عمره انجام دهم؟",
        fr: "Comment effectuer l'Umrah ?",
        de: "Wie führe ich die Umrah durch?",
      },
      {
        en: "When is the next prayer?",
        ar: "متى الصلاة القادمة؟",
        ur: "اگلی نماز کب ہے؟",
        hi: "अगली नमाज़ कब है?",
        id: "Kapan waktu shalat berikutnya?",
        ms: "Bilakah waktu solat seterusnya?",
        tr: "Bir sonraki namaz ne zaman?",
        fa: "نماز بعدی چه وقت است؟",
        fr: "Quand est la prochaine prière ?",
        de: "Wann ist das nächste Gebet?",
      },
      {
        en: "How do I perform Talbiyah?",
        ar: "كيف أؤدي التلبية؟",
        ur: "تلبیہ کیسے کریں؟",
        hi: "तलबिया कैसे करें?",
        id: "Bagaimana cara melakukan Talbiyah?",
        ms: "Bagaimana cara melakukan Talbiyah?",
        tr: "Telbiye nasıl yapılır?",
        fa: "چگونه تلبیه کنم؟",
        fr: "Comment faire la Talbiyah ?",
        de: "Wie führe ich die Talbiyah durch?",
      },
    ],
  },
  facilities: {
    title: "Facilities & Services",
    phrases: [
      {
        en: "Where is the nearest bathroom?",
        ar: "أين أقرب دورة مياه؟",
        ur: "قریب ترین باتھ روم کہاں ہے؟",
        hi: "निकटतम शौचालय कहाँ है?",
        id: "Di mana kamar mandi terdekat?",
        ms: "Di manakah tandas yang terdekat?",
        tr: "En yakın tuvalet nerede?",
        fa: "نزدیک‌ترین دستشویی کجاست؟",
        fr: "Où sont les toilettes les plus proches ?",
        de: "Wo ist die nächste Toilette?",
      },
      {
        en: "Where can I find Zamzam water?",
        ar: "أين يمكنني الحصول على ماء زمزم؟",
        ur: "زمزم کا پانی کہاں سے مل سکتا ہے؟",
        hi: "ज़मज़म का पानी कहाँ मिल सकता है?",
        id: "Di mana saya bisa mendapatkan air Zamzam?",
        ms: "Di mana saya boleh mendapatkan air Zamzam?",
        tr: "Zemzem suyunu nereden bulabilirim?",
        fa: "کجا می‌توانم آب زمزم پیدا کنم؟",
        fr: "Où puis-je trouver de l'eau de Zamzam ?",
        de: "Wo finde ich Zamzam-Wasser?",
      },
      {
        en: "Where is the nearest medical center?",
        ar: "أين أقرب مركز طبي؟",
        ur: "قریب ترین طبی مرکز کہاں ہے؟",
        hi: "निकटतम चिकित्सा केंद्र कहाँ है?",
        id: "Di mana pusat kesehatan terdekat?",
        ms: "Di manakah pusat perubatan yang terdekat?",
        tr: "En yakın sağlık merkezi nerede?",
        fa: "نزدیک‌ترین مرکز پزشکی کجاست؟",
        fr: "Où est le centre médical le plus proche ?",
        de: "Wo ist das nächste medizinische Zentrum?",
      },
      {
        en: "Where can I find a prayer mat?",
        ar: "أين يمكنني العثور على سجادة صلاة؟",
        ur: "نماز کی چٹائی کہاں سے مل سکتی ہے؟",
        hi: "नमाज़ की चटाई कहाँ मिल सकती है?",
        id: "Di mana saya bisa mendapatkan sajadah?",
        ms: "Di mana saya boleh mendapatkan sejadah?",
        tr: "Secdeyi nereden bulabilirim?",
        fa: "کجا می‌توانم جانماز پیدا کنم؟",
        fr: "Où puis-je trouver un tapis de prière ?",
        de: "Wo finde ich eine Gebetsmatte?",
      },
      {
        en: "Where is the nearest ATM?",
        ar: "أين أقرب ماكينة صراف آلي؟",
        ur: "قریب ترین اے ٹی ایم کہاں ہے؟",
        hi: "निकटतम एटीएम कहाँ है?",
        id: "Di mana ATM terdekat?",
        ms: "Di manakah ATM yang terdekat?",
        tr: "En yakın ATM nerede?",
        fa: "نزدیک‌ترین دستگاه خودپرداز کجاست؟",
        fr: "Où est le distributeur automatique le plus proche ?",
        de: "Wo ist der nächste Geldautomat?",
      },
      {
        en: "Where can I find a wheelchair?",
        ar: "أين يمكنني العثور على كرسي متحرك؟",
        ur: "وہیل چیئر کہاں سے مل سکتی ہے؟",
        hi: "व्हीलचेयर कहाँ मिल सकती है?",
        id: "Di mana saya bisa mendapatkan kursi roda?",
        ms: "Di mana saya boleh mendapatkan kerusi roda?",
        tr: "Tekerlekli sandalye nereden bulabilirim?",
        fa: "کجا می‌توانم ویلچر پیدا کنم؟",
        fr: "Où puis-je trouver un fauteuil roulant ?",
        de: "Wo finde ich einen Rollstuhl?",
      },
      {
        en: "Where can I find a prayer room?",
        ar: "أين يمكنني العثور على غرفة صلاة؟",
        ur: "نماز کی جگہ کہاں مل سکتی ہے؟",
        hi: "प्रार्थना कक्ष कहाँ मिल सकता है?",
        id: "Di mana saya bisa menemukan ruang shalat?",
        ms: "Di mana saya boleh mencari bilik solat?",
        tr: "Namaz odası nerede bulabilirim?",
        fa: "کجا می‌توانم اتاق نماز پیدا کنم؟",
        fr: "Où puis-je trouver une salle de prière ?",
        de: "Wo finde ich einen Gebetsraum?",
      },
      {
        en: "Where is the nearest restaurant?",
        ar: "أين أقرب مطعم؟",
        ur: "قریب ترین ریستوران کہاں ہے؟",
        hi: "निकटतम रेस्तरां कहाँ है?",
        id: "Di mana restoran terdekat?",
        ms: "Di manakah restoran yang terdekat?",
        tr: "En yakın restoran nerede?",
        fa: "نزدیک‌ترین رستوران کجاست؟",
        fr: "Où est le restaurant le plus proche ?",
        de: "Wo ist das nächste Restaurant?",
      },
      {
        en: "Where can I find a currency exchange?",
        ar: "أين يمكنني العثور على صرافة عملات؟",
        ur: "کرنسی ایکسچینج کہاں مل سکتا ہے؟",
        hi: "मुद्रा विनिमय कहाँ मिल सकता है?",
        id: "Di mana saya bisa menemukan penukaran mata uang?",
        ms: "Di mana saya boleh mencari pertukaran mata wang?",
        tr: "Döviz bürosu nerede bulabilirim?",
        fa: "کجا می‌توانم صرافی پیدا کنم؟",
        fr: "Où puis-je trouver un bureau de change ?",
        de: "Wo finde ich eine Wechselstube?",
      },
    ],
  },
  emergency: {
    title: "Emergency & Medical",
    phrases: [
      {
        en: "I need medical assistance",
        ar: "أحتاج إلى مساعدة طبية",
        ur: "مجھے طبی امداد چاہیے",
        hi: "मुझे चिकित्सा सहायता चाहिए",
        id: "Saya butuh bantuan medis",
        ms: "Saya perlukan bantuan perubatan",
        tr: "Tıbbi yardıma ihtiyacım var",
        fa: "من به کمک پزشکی نیاز دارم",
        fr: "J'ai besoin d'assistance médicale",
        de: "Ich brauche medizinische Hilfe",
      },
      {
        en: "I need water",
        ar: "أحتاج إلى ماء",
        ur: "مجھے پانی چاہیے",
        hi: "मुझे पानी चाहिए",
        id: "Saya butuh air",
        ms: "Saya perlukan air",
        tr: "Suya ihtiyacım var",
        fa: "من به آب نیاز دارم",
        fr: "J'ai besoin d'eau",
        de: "Ich brauche Wasser",
      },
      {
        en: "I feel dizzy",
        ar: "أشعر بالدوار",
        ur: "مجھے چکر آ رہے ہیں",
        hi: "मुझे चक्कर आ रहे हैं",
        id: "Saya merasa pusing",
        ms: "Saya berasa pening",
        tr: "Başım dönüyor",
        fa: "من احساس سرگیجه می‌کنم",
        fr: "Je me sens étourdi",
        de: "Mir ist schwindelig",
      },
      {
        en: "I lost my group",
        ar: "لقد فقدت مجموعتي",
        ur: "میں اپنے گروپ سے بچھڑ گیا ہوں",
        hi: "मैं अपने समूह से बिछड़ गया हूं",
        id: "Saya kehilangan grup saya",
        ms: "Saya telah kehilangan kumpulan saya",
        tr: "Grubumu kaybettim",
        fa: "من گروه خود را گم کرده‌ام",
        fr: "J'ai perdu mon groupe",
        de: "Ich habe meine Gruppe verloren",
      },
      {
        en: "I need first aid",
        ar: "أحتاج إلى إسعافات أولية",
        ur: "مجھے فوری طبی امداد چاہیے",
        hi: "मुझे प्राथमिक चिकित्सा चाहिए",
        id: "Saya butuh pertolongan pertama",
        ms: "Saya perlukan pertolongan cemas",
        tr: "İlk yardıma ihtiyacım var",
        fa: "من به کمک‌های اولیه نیاز دارم",
        fr: "J'ai besoin de premiers soins",
        de: "Ich brauche Erste Hilfe",
      },
      {
        en: "I lost my passport",
        ar: "لقد فقدت جواز سفري",
        ur: "میں نے اپنا پاسپورٹ کھو دیا ہے",
        hi: "मैंने अपना पासपोर्ट खो दिया है",
        id: "Saya kehilangan paspor saya",
        ms: "Saya telah kehilangan pasport saya",
        tr: "Pasaportumu kaybettim",
        fa: "من پاسپورتم را گم کرده‌ام",
        fr: "J'ai perdu mon passeport",
        de: "Ich habe meinen Pass verloren",
      },
      {
        en: "I need an ambulance",
        ar: "أحتاج إلى سيارة إسعاف",
        ur: "مجھے ایمبولنس چاہیے",
        hi: "मुझे एम्बुलेंस चाहिए",
        id: "Saya butuh ambulans",
        ms: "Saya perlukan ambulans",
        tr: "Ambulansa ihtiyacım var",
        fa: "من به آمبولانس نیاز دارم",
        fr: "J'ai besoin d'une ambulance",
        de: "Ich brauche einen Krankenwagen",
      },
      {
        en: "I lost my wallet",
        ar: "لقد فقدت محفظتي",
        ur: "میں نے اپنا پرس کھو دیا ہے",
        hi: "मैंने अपना बटुआ खो दिया है",
        id: "Saya kehilangan dompet saya",
        ms: "Saya telah kehilangan dompet saya",
        tr: "Cüzdanımı kaybettim",
        fa: "من کیف پولم را گم کرده‌ام",
        fr: "J'ai perdu mon portefeuille",
        de: "Ich habe meine Brieftasche verloren",
      },
      {
        en: "I need a doctor",
        ar: "أحتاج إلى طبيب",
        ur: "مجھے ڈاکٹر چاہیے",
        hi: "मुझे डॉक्टर चाहिए",
        id: "Saya butuh dokter",
        ms: "Saya perlukan doktor",
        tr: "Doktora ihtiyacım var",
        fa: "من به پزشک نیاز دارم",
        fr: "J'ai besoin d'un médecin",
        de: "Ich brauche einen Arzt",
      },
    ],
  },
  shopping: {
    title: "Shopping & Purchases",
    phrases: [
      {
        en: "How much does this cost?",
        ar: "كم تكلفة هذا؟",
        ur: "اس کی قیمت کتنی ہے؟",
        hi: "इसकी कीमत कितनी है?",
        id: "Berapa harganya?",
        ms: "Berapakah harganya?",
        tr: "Bu ne kadar?",
        fa: "قیمت این چقدر است؟",
        fr: "Combien ça coûte ?",
        de: "Wie viel kostet das?",
      },
      {
        en: "Do you accept credit cards?",
        ar: "هل تقبلون بطاقات الائتمان؟",
        ur: "کیا آپ کریڈٹ کارڈ قبول کرتے ہیں؟",
        hi: "क्या आप क्रेडिट कार्ड स्वीकार करते हैं?",
        id: "Apakah Anda menerima kartu kredit?",
        ms: "Adakah anda menerima kad kredit?",
        tr: "Kredi kartı kabul ediyor musunuz?",
        fa: "آیا کارت اعتباری قبول می‌کنید؟",
        fr: "Acceptez-vous les cartes de crédit ?",
        de: "Akzeptieren Sie Kreditkarten?",
      },
      {
        en: "Can I get a discount?",
        ar: "هل يمكنني الحصول على خصم؟",
        ur: "کیا مجھے رعایت مل سکتی ہے؟",
        hi: "क्या मुझे छूट मिल सकती है?",
        id: "Bisakah saya mendapatkan diskon?",
        ms: "Bolehkah saya mendapat diskaun?",
        tr: "İndirim alabilir miyim?",
        fa: "آیا می‌توانم تخفیف بگیرم؟",
        fr: "Puis-je avoir une réduction ?",
        de: "Kann ich einen Rabatt bekommen?",
      },
      {
        en: "Where can I buy Ihram?",
        ar: "أين يمكنني شراء الإحرام؟",
        ur: "احرام کہاں سے خرید سکتا ہوں؟",
        hi: "मैं इहराम कहाँ से खरीद सकता हूं?",
        id: "Di mana saya bisa membeli Ihram?",
        ms: "Di mana saya boleh membeli Ihram?",
        tr: "İhram nereden alabilirim?",
        fa: "کجا می‌توانم احرام بخرم؟",
        fr: "Où puis-je acheter l'Ihram ?",
        de: "Wo kann ich Ihram kaufen?",
      },
      {
        en: "Do you have prayer beads?",
        ar: "هل لديكم مسبحة؟",
        ur: "کیا آپ کے پاس تسبیح ہے؟",
        hi: "क्या आपके पास प्रार्थना माला है?",
        id: "Apakah Anda memiliki tasbih?",
        ms: "Adakah anda mempunyai tasbih?",
        tr: "Tesbihiniz var mı?",
        fa: "آیا تسبیح دارید؟",
        fr: "Avez-vous des chapelets de prière ?",
        de: "Haben Sie Gebetsperlen?",
      },
      {
        en: "Where can I buy dates?",
        ar: "أين يمكنني شراء التمر؟",
        ur: "کھجوریں کہاں سے خرید سکتا ہوں؟",
        hi: "मैं खजूर कहाँ से खरीद सकता हूं?",
        id: "Di mana saya bisa membeli kurma?",
        ms: "Di mana saya boleh membeli kurma?",
        tr: "Hurma nereden alabilirim?",
        fa: "کجا می‌توانم خرما بخرم؟",
        fr: "Où puis-je acheter des dattes ?",
        de: "Wo kann ich Datteln kaufen?",
      },
      {
        en: "Do you have prayer rugs?",
        ar: "هل لديكم سجادات صلاة؟",
        ur: "کیا آپ کے پاس نماز کی چٹائیاں ہیں؟",
        hi: "क्या आपके पास प्रार्थना चटाई हैं?",
        id: "Apakah Anda memiliki sajadah?",
        ms: "Adakah anda mempunyai sejadah?",
        tr: "Secde yastığınız var mı?",
        fa: "آیا جانماز دارید؟",
        fr: "Avez-vous des tapis de prière ?",
        de: "Haben Sie Gebetsteppiche?",
      },
      {
        en: "Where can I buy Abaya?",
        ar: "أين يمكنني شراء عباية؟",
        ur: "عبایہ کہاں سے خرید سکتا ہوں؟",
        hi: "मैं अबाया कहाँ से खरीद सकता हूं?",
        id: "Di mana saya bisa membeli Abaya?",
        ms: "Di mana saya boleh membeli Abaya?",
        tr: "Abiye nereden alabilirim?",
        fa: "کجا می‌توانم عبایه بخرم؟",
        fr: "Où puis-je acheter une Abaya ?",
        de: "Wo kann ich eine Abaya kaufen?",
      },
      {
        en: "Do you have Islamic books?",
        ar: "هل لديكم كتب إسلامية؟",
        ur: "کیا آپ کے پاس اسلامی کتابیں ہیں؟",
        hi: "क्या आपके पास इस्लामी किताबें हैं?",
        id: "Apakah Anda memiliki buku-buku Islam?",
        ms: "Adakah anda mempunyai buku-buku Islam?",
        tr: "İslami kitaplarınız var mı?",
        fa: "آیا کتاب‌های اسلامی دارید؟",
        fr: "Avez-vous des livres islamiques ?",
        de: "Haben Sie islamische Bücher?",
      },
    ],
  },
};
